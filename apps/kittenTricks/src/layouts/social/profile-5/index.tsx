...
import { Text } from '@meow-ui/components';
...
export default (): React.ReactElement => {
  ...
  return (
    <ImageOverlay style={styles.container} source={profile.photo}>
      <View style={styles.profileDetailsContainer}>
        <Text style={styles.profileName} category='h1' status='control'>
          {profile.fullName}
        </Text>
        <Text style={styles.profileLocation} category='h6' status='control'>
          {profile.location}
        </Text>
        <RateBar hint='Experience' value={rating} onValueChange={setRating} />
