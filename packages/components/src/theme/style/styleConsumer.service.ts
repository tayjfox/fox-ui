...
import { ControlMetaType, ControlThemedStyleType, ThemedStyleType } from '@eva-design/dss';

import { ThemeType } from '../theme/theme.service';
import { Interaction, StyleService, StyleType } from './style.service';
...
export class StyleConsumerService {
  ...
  public createStyleProp<P extends object>(
    source: P,
    style: StyleType,
    theme: ThemeType,
    ...
    if (!generatedMapping) {
      ...
      return this.createStyleProp(
        { ...source, ...this.createDefaultProps() },
        style,
        theme,
        interaction
      );
  ...
  private getGeneratedStyleMapping(style: StyleType, info: StyleInfo): StyleType {
    return this.safe(
      style[this.name],
      (componentStyles: ControlThemedStyleType): ThemedStyleType => {
  ...
  private getStyleInfo<P extends StyledComponentProps>(
    props: P,
    interaction: Interaction[]
  ): StyleInfo {
  ...
  private getDefaultAppearance(): string {
    const matches: string[] = Object.keys(this.meta.appearances).filter(
      (appearance: string): boolean => {
  ...
  private getDefaultVariants(): DefaultVariants {
    return this.transformObject(
      this.meta.variantGroups,
      (variants, group: string): string | undefined => {
  ...
  private getDerivedVariants<P extends StyledComponentProps>(
  ...
  private getDerivedStates<P extends StyledComponentProps>(
    meta: ControlMetaType,
    props: P
  ): Partial<P> {
   ...
   */
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  private transformObject<V extends object>(
    value: V,
  ...
  private findGeneratedQuery(info: StyleInfo, source: string[]): string | undefined {
    const query: string[] = [info.appearance, ...info.variants, ...info.states];
  ...
  private compareArrays(lhs: string[], rhs: string[]): boolean {
    ...
    return lhs.reduce(
      (acc: boolean, current: string): boolean => acc && rhs.includes(current),
      true
    );
  }
